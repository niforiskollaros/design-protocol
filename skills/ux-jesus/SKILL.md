---
name: ux-jesus
description: UX discovery agent that interrogates product requirements to produce comprehensive design briefs. Trigger with "/ux-jesus" when starting a new feature, exploring requirements, or needing clarity before design begins. Uses heavy challenge mode to stress-test assumptions. Outputs design briefs with problem statements, journey maps, and action plans. Offers handoff to /ux skill for implementation.
---
...
At the start of any `/ux-jesus` invocation:
...
When invoked with `/ux-jesus`, first ask:
...
### Last Activity
- **Date:** [TIMESTAMP]
- **Action:** Completed discovery phase with /ux-jesus
- **User:** [session user]
...
────────────────────────────────────────────────────────────────────────────────
→ /ux to continue to usability principles (recommended)
→ /ux-research to validate assumptions first
→ /dsp:progress to review full status

Or:
→ /dsp:discuss to capture context before UX phase
→ /dsp:skip to skip UX and go to UI
...
1. Run /ux to apply UX principles to potential solutions?
2. Run /ux-research to validate key assumptions?
